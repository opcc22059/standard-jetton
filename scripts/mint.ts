...
//   }
  ...
  const minterAddressStr = config.minter;
  ...
  console.log("minter raw address: ", minterAddress.toRawString())
  const minterContract = client.open(minter);

  const toAddress = Address.parse(config.mint_recipient);
  const fwdValue = BigInt(Number(config.mint_fwd_value) * 1e9)
  const jettonAmount = BigInt(Number(config.mint_jetton_amount) * 1e9)

  // send the increment transaction
  minterContract.sendMint(walletSender, 1, fwdValue, jettonAmount, toAddress, "0.02")
